...
HRESULT _stdcall Hooked_Present(IDirect3DDevice9* pDevice, const RECT* pSourceRect, const RECT* pDestRect, HWND hDestWindowOverride, const RGNDATA* pDirtyRegion) {
	...
	if (toggleHitbox || g_framebar_toggled) {
		pDevice->BeginScene();
		if (toggleHitbox) {
			DWORD state;
		...
		if (g_framebar_toggled) {
			DrawFrameBar(pDevice, *resolutionX, *resolutionY);
		}
		pDevice->EndScene();
	}
	...
	if (frameStep == true) {
		...
		else if (GetAsyncKeyState(VK_F8) && keyPressed == 1) {
			g_framebar_toggled = !g_framebar_toggled;
		}
		else
...
HRESULT _stdcall Hooked_Reset(IDirect3DDevice9* pDevice, D3DPRESENT_PARAMETERS* pPresentationParameters) {
	if (m_font) {
		m_font->OnLostDevice();
	}
...
DWORD WINAPI MainThread(LPVOID hModule)
{
	...
	DetourAttach(&(LPVOID&)oReset, Hooked_Reset);
